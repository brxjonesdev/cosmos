...
export default async function RepoList() {
  ...
  async function getDatabaseRepos() {
    const { data, error } = await supabase
      .from('Repos')
      .select()
      .eq('repo_owner', user?.user?.user_metadata?.user_name);
    if (error) {
      console.log(error);
      return [];
    }
  ...
  if (!githubRepos || githubRepos === undefined) {
    redirect('/');
  }
      ...
      <div className="flex justify-between">
        <CardHeader className="pb-0">
          <CardTitle className="text-3xl">
            Welcome In, {user.user.user_metadata.name}{' '}
            <span className="text-sm font-normal">{`( ${user.user.user_metadata.user_name} )`}</span>
          </CardTitle>
          <CardDescription className="">
            You have{' '}
            {CosmosRepos.databaseRepos.length !== 0
              ? CosmosRepos.databaseRepos.length
              : 'no'}{' '}
            repo{CosmosRepos.databaseRepos.length == 1 ? '' : 's'} connected to
            Cosmos.{' '}
          </CardDescription>
        </CardHeader>
        <CardContent className="py-0 self-end "></CardContent>
      </div>
      <CardContent className="overflow-y-scroll flex-grow border-2 m-6 space-y-4 py-6 bg-black/10 rounded-md max-h-[700px] ">
        <h2 className="font-medium">Connected Repos</h2>
        {CosmosRepos.databaseRepos.map((repo: any) => {
          return <Repo key={repo.repo_name} repo={repo} />;
        })}
        <h2 className="font-medium">Unconnected Repos</h2>
        {CosmosRepos.githubRepos.map((repo: SimpleRepo) => {
          return <Repo key={repo.name} repo={repo} />;
        })}

        {CosmosRepos.githubRepos.length === 0 &&
          CosmosRepos.databaseRepos.length === 0 && (
